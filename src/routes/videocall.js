...
// Create a secure room for a consultation
router.post('/create/:consultationId', auth, async (req, res) => {
    try {
        ...
        if (consultation.roomName) {
            return res.json({ roomName: consultation.roomName, url: `https://meet.jit.si/${consultation.roomName}` })
        }
...
router.get('/:consultationId', auth, async (req, res) => {
    try {
        ...
        if (!consultation.roomName) {
            return res.status(404).json({ error: 'No room created yet' })
        }
