...
router.post('/register', async (req, res) => {
    try {
        ...
        res.status(201).json({
            token,
            doctor: { id: doctor.id, name: doctor.name, email: doctor.email, specialty: doctor.specialty, hospital: doctor.hospital, verified: doctor.verified }
        })
    } catch (err) {
...
router.get('/me', async (req, res) => {
    try {
        ...
        const doctor = await db.doctor.findUnique({
            where: { id: decoded.id },
            select: { id: true, name: true, email: true, specialty: true, hospital: true, verified: true, reputation: true, cmeCredits: true }
        })
        res.json(doctor)
    } catch (err) {
        console.error(err)
